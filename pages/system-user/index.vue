<template>
  <div class="flex-1 overflow-y-auto p-4 md:p-8">
      ...
      >
        <div>
          ...
          <p class="text-sm text-slate-500">
            Manage administrative access levels and staff credentials.
          </p>
        </div>
        <button
          @click="openCreate"
      ...
      >
        <template #cell-name="{ row }">
          <div class="flex items-center gap-3">
            <div
              class="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 font-bold text-xs border border-slate-200"
            >
              {{ row.name.charAt(0) }}
            </div>
        ...
        <template #cell-status="{ row }">
          <div class="flex items-center gap-2">
            ...
            <span
              ...
              :class="row.isActive ? 'text-slate-900' : 'text-slate-400'"
            >
    ...
    <DeleteConfirmModal
      v-model="showDelete"
      title="Delete User"
      :description="`Are you sure you want to delete '${selectedUser?.name}'? This cannot be undone.`"
...
import { Pencil, Trash2, UserPlus, ShieldCheck, Users } from "lucide-vue-next";
...
const handleUserSubmit = (formData) => {
  ...
  const branchName =
    formData.branchId === 0 ? "All Branches" : branchObj?.name || "Unknown";
