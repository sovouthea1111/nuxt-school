...
function defineColadaLoader(nameOrOptions, _options) {
	...
	function load(to, router, from, parent, reload) {
		...
		if (!entry.ext) {
			...
			(0, __pinia_colada.useQueryCache)().getEntries({
				exact: true,
				key: toValueWithParameters(options.key, to)
			})[0]?.deps.delete(router[unplugin_vue_router_data_loaders.DATA_LOADERS_EFFECT_SCOPE_KEY]);
			reload = false;
		}
